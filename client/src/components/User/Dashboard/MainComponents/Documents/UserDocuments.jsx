...
const UserDocuments = () => {
  ...
  useEffect(() => {
    const func = async () => {
      ...
      for (let i = 0; i < allDocuments[0].length; i++) {
        objects.push({
          id: i + 1,
          issuer: allDocuments[0][i],
          ipfsHash: allDocuments[1][i],
          doctype: allDocuments[2][i],
          timestamp: new Date(parseInt(allDocuments[3][i], 10) * 1000).toDateString(),
        });
      }
    ...
    if (instance.methods.getAllDocuments) {
      func();
    }
  }, [instance]);
  ...
  const showData = tableData.slice(pagesVisited, pagesVisited + dataPerPage).map((val, index) => (
    <tr key={val.id} className={index % 2 !== 0 ? 'bg-blue-100' : 'bg-white'}>
      ...
      <td className="font-ubuntu p-2">{val.issuer}</td>
