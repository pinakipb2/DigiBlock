...
const AccessRejected = () => {
  ...
  useEffect(() => {
    const func = async () => {
      const rejectedDocs = await instance.methods.getUserRejectedDocuments(userAddress).call();
      for (let i = 0; i < rejectedDocs[0].length; i++) {
        if (rejectedDocs[2][i] !== '0') {
          objects.push({
            id: i + 1,
            requestorAddress: rejectedDocs[0][i],
    ...
    <div className="px-6 pb-10">
      <div className="text-white flex justify-between items-center bg-gray-800 w-full text-xl p-4 mb-1.5">
        <div className="font-ubuntu">
          Access
          <span className="font-bold ml-2">Rejected</span>
        </div>
