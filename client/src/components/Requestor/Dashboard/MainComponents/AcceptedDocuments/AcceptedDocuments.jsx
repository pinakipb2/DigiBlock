  ...
  const searchTable = (term) => {
    ...
    if (searchTerm === '') {
      ...
      const filteredTable = originalData.filter(
        (row) => row.userAddress.toLowerCase().includes(searchTerm) || row.docType.toLowerCase().includes(searchTerm) || row.ipfsHash.toLowerCase().includes(searchTerm) || row.timestamp.toLowerCase().includes(searchTerm)
      );
